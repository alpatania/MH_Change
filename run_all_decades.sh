...
# Per-search home: <out-dir>/results/<search-slug>/. --out-dir is the BASE
# location (default: current directory), and results/<search>/ is always
# appended, so distinct searches never collide and you can't accidentally
# point two searches at one flat folder. Files keep the <search>_ prefix.
